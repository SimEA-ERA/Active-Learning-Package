...
main_set_of_files_path="ActiveLearningFiles"  # Assuming Python scripts are in the same directory as this script
...
if [ "$contin" -eq 0 ]; then
  > $eval_dir/mae.dat
  > $eval_dir/mse.dat
  > $eval_dir/elasticnet.dat
  > $eval_dir/pred_elasticnet.dat
